...
namespace WebApiClientCore.Attributes
{
    ...
    public abstract class ClientCredentialsTokenFilterAttribue : TokenFilterAttribute
    {
        ...
        protected override Task<TokenResult> RequestTokenAsync(ApiRequestContext context)
        {
            ...
            return tokenClient.RequestClientCredentialsAsync(credentials.ClientId, credentials.ClientSecret, credentials.Scope, credentials.Extra);
        }
        ...
        /// <returns></returns>
        protected override Task<TokenResult> RequestRefreshTokenAsync(ApiRequestContext context, string refresh_token)
        {
            var credentials = this.GetClientCredentials(context);
            var tokenClient = new TokenClient(credentials.TokenEndpoint);
            return tokenClient.RequestRefreshTokenAsync(credentials.ClientId, credentials.ClientSecret, refresh_token, credentials.Extra);
        }
