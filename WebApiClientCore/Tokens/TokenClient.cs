...
namespace WebApiClientCore.Tokens
{
    ...
    public class TokenClient
    {
        ...
        private async Task<TokenResult> RequestTokenResultAsync(Credentials credentials)
        {
            ...
            var response = await httpClient.PostAsync(this.TokenEndpoint, formContent).ConfigureAwait(false);
            var json = await response.Content.ReadAsByteArrayAsync().ConfigureAwait(false);
            var token = jsonFormatter.Deserialize(json, typeof(TokenResult), HttpApiOptions.CreateDefaultJsonOptions());
